...
function [rec, finalResults, results, recoveredSignals] = benchmarkSynthetic(xitem, xvalues, yitem, algorithms, dataSet, params, opt)
...
%     fprintf('Running trails: %s=%g\n',xitem,xvalues(p))
    ...
    for k=1:length(algorithms)
        ...
        fprintf('  %s begin...',algorithms{k}.algorithm);
        if numTrials==1
            fprintf('\n');
        end
        ...
        for q=1:numTrials
            ...
            [A, At, b0, xt, plotter, params] = createProblemData(dataSet, params, opt);
            n = numel(xt);
            opts.xt = xt;
...
%       dataset.
function [A, At, b0, xt, plotter, params] = createProblemData(dataSet, params, opt)
